...
public class RandomUtil {
    ...
    private static String en = "abcdefghijklmnopqrstuvwxyz";
    ...
    public static String getEmail(int length) {
        ...
        for (int i = 0; i < length; i++) {
            sb.append(en.charAt(random.nextInt(length)));
        }
    ...
    public static String getName() {
        ...
        int first = random.nextInt(firstName.length() - 1);
        sb.append(firstName.substring(first, first + 1));
        int sex = random.nextInt(2);
        if (sex == 0) {
            int second = random.nextInt(boy.length() - 1);
            int third = random.nextInt(boy.length() - 1);
            sb.append(girl.substring(second, second + 1));
            if (random.nextBoolean()) {
                sb.append(girl.substring(third, third + 1));
            }
            name_sex = "女";
        } else if (sex == 1) {
            int second = random.nextInt(girl.length() - 1);
            int third = random.nextInt(girl.length() - 1);
            sb.append(boy.substring(second, second + 1));
            if (random.nextBoolean()) {
                sb.append(boy.substring(third, third + 1));
            }
            name_sex = "男";
        }
        return sb.toString();
    ...
    public static Map getUserMap() {
        ...
        map.put("tel", getTel());
        map.put("email", getEmail(6));
        return map;
    }

    public static String getUserInfo() {
        StringBuilder sb = new StringBuilder();
        sb.append(getName() + "  " + name_sex + "  " + getRoad() + "  " + getTel() + "  " + getEmail(6));
        return sb.toString();
    }
    ...
    public static String getIp() {
        ...
        String ip = numToIp(range[index][0] + new Random().nextInt(range[index][1] - range[index][0]));
        return ip;
    }
